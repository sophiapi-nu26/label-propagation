    ...
    int n, double p, double q, std::mt19937 &gen)
{
    ...
    for (int i = 0; i < n; i++) {
        community[i] = (communityDist(gen) ? 1 : 0);
    }
    ...
    // // DEBUG write adj to file and save as .txt
    // std::ostringstream adjFilename;
    // adjFilename << "adj_" << p << "_" << q << ".txt";
    // std::ofstream adjFile(adjFilename.str());
    // for (int i = 0; i < n; i++) {
    //     adjFile << i << ": ";
    //     for (int j : adj[i]) {
    //         adjFile << j << " ";
    ...
    //     adjFile << "\n";
    // }
    // // DEBUG write community to file and save as .txt
    // std::ostringstream communityFilename;
    // communityFilename << "community_" << p << "_" << q << ".txt";
    // std::ofstream communityFile(communityFilename.str());
    // for (int i = 0; i < n; i++) {
    //     communityFile << i << ": " << community[i] << "\n";
    // }
    // communityFile.close();
    // adjFile.close();
...
static void runOneSimulation(
    ...
    int numiterations,
    std::mt19937 &gen,
    // local arrays to fill for each round in [0..numiterations-1]
    ...
    // // DEBUG write labels to file and save as .txt
    // std::ostringstream labelsFilename;
    // labelsFilename << "labels_" << p << "_" << q << "_origiinal.txt";
    // std::ofstream labelsFile(labelsFilename.str());
    // for (int i = 0; i < n; i++) {
    //     labelsFile << i << ": " << labels[i] << "\n";
    // }

    // ------------------------------------------------------------------------
    // Force label=1 to be in community 0 (so that community 0 definitely starts
    // with the smallest label). Same approach as before:
    // {
        int idxOfLabel1 = -1;
        int idxInC0 = -1;
        for (int u = 0; u < n; u++) {
        ...
        if (idxOfLabel1 >= 0 && community[idxOfLabel1] == 1) {
            ...
            if (idxInC0 >= 0) {
                int tmp = labels[idxInC0];
                labels[idxInC0] = 1;
                labels[idxOfLabel1] = tmp;
            }
        }

        
        // // DEBUG save labels to file and save as .txt
        // std::ostringstream labelsFilename;
        // labelsFilename << "labels_" << p << "_" << q << "_after_force_label_1_in_c0.txt";
        // std::ofstream labelsFile(labelsFilename.str());
        // labelsFile << "idxOfLabel1: " << idxOfLabel1 << "\n";
        // labelsFile << "community[idxOfLabel1]: " << community[idxOfLabel1] << "\n";
        // labelsFile << "labels[idxOfLabel1]: " << labels[idxOfLabel1] << "\n"; // should be 1
        // labelsFile << "community[idxInC0]: " << community[idxInC0] << "\n";
        // labelsFile << "labels[idxInC0]: " << labels[idxInC0] << "\n"; // should be 1
        // for (int i = 0; i < n; i++) {
        //     labelsFile << i << ": " << labels[i] << "\n";
        // }
        // labelsFile.close();
    ...
    // 3) Determine the smallest initial label in each community
    int initMinLabelC0 = INT32_MAX; // should be 1
    int initMinLabelC1 = INT32_MAX; // could be anything >1
    ...
    // Find the min label in each community
    for (int u = 0; u < n; u++) {
    ...
    if (c0count == 0) initMinLabelC0 = -1; // empty
    if (c1count == 0) initMinLabelC1 = -1; // empty
    // possible TODO: debug check that initMinLabelC0 is 1

    // We'll track old labels to see who changes from round to round
    std::vector<int> prevLabels(n, -1);

    // A lambda to compute all stats after each round
    auto computeRoundStats = [&](int roundIdx) {
        // We'll count how many nodes in c0 have initMinLabelC0, etc.
        int countC0_minC0 = 0;
        int countC1_minC1 = 0;

        // For conv_smallest_global: fraction of c0 or c1 that hold label=1
        int count_sm_global_c0 = 0, count_sm_global_c1 = 0;

        // fraction_not_changed
        int count_not_changed_c0 = 0, count_not_changed_c1 = 0;

        // cross_label_distribution (for c0 and c1)
        int c0_mC0 = 0, c0_mC1 = 0; // how many in c0 have initMinLabelC0 or C1
        int c1_mC0 = 0, c1_mC1 = 0; // how many in c1 have initMinLabelC0 or C1
        ...
        for (int u = 0; u < n; u++) {
            ...
            //   For c0: how many in c0 have initMinLabelC0
            //   For c1: how many in c1 have initMinLabelC1
            if (comm == 0 && lbl == initMinLabelC0) countC0_minC0++;
            if (comm == 1 && lbl == initMinLabelC1) countC1_minC1++;

            // conv_smallest_global (fraction of each community with label=1)
            ...
            // For c0 => how many have initMinLabelC0 or initMinLabelC1`
            // For c1 => how many have initMinLabelC0 or initMinLabelC1
            if (comm == 0) {
                if (lbl == initMinLabelC0) c0_mC0++;
                if (lbl == initMinLabelC1) c0_mC1++;
            } else {
                // comm == 1
                if (lbl == initMinLabelC0) c1_mC0++;
                if (lbl == initMinLabelC1) c1_mC1++;
            }
        }

        // Store results in the local arrays:

        // conv_smallest_in_comm: fraction of nodes in c0 that have initMinLabelC0
        //                        fraction of nodes in c1 that have initMinLabelC1
        // So the denominator is c0count or c1count
        double dc0 = double(c0count);
        double dc1 = double(c1count);

        local_csc0[roundIdx] = (dc0 > 0 && initMinLabelC0 != -1)
            ? double(countC0_minC0)/dc0
            : 0.0;
        local_csc1[roundIdx] = (dc1 > 0 && initMinLabelC1 != -1)
            ? double(countC1_minC1)/dc1
            : 0.0;

        // conv_smallest_global => fraction of c0 or c1 with label=1
        local_csg0[roundIdx] = (dc0 > 0) 
            ? double(count_sm_global_c0)/dc0 : 0.0;
        local_csg1[roundIdx] = (dc1 > 0)
            ? double(count_sm_global_c1)/dc1 : 0.0;

        // fraction_not_changed => fraction of c0 or c1 that didn't change
        local_fnc0[roundIdx] = (roundIdx > 0 && dc0 > 0)
            ? double(count_not_changed_c0)/dc0 : 0.0;
        local_fnc1[roundIdx] = (roundIdx > 0 && dc1 > 0)
            ? double(count_not_changed_c1)/dc1 : 0.0;

        // cross_label_distribution => fraction of c0 or c1 with initMinLabelC0 or initMinLabelC1
        local_cld00[roundIdx] = (dc0 > 0 && initMinLabelC0 != -1)
            ? double(c0_mC0)/dc0 : 0.0;
        local_cld01[roundIdx] = (dc0 > 0 && initMinLabelC1 != -1)
            ? double(c0_mC1)/dc0 : 0.0;
        local_cld10[roundIdx] = (dc1 > 0 && initMinLabelC0 != -1)
            ? double(c1_mC0)/dc1 : 0.0;
        local_cld11[roundIdx] = (dc1 > 0 && initMinLabelC1 != -1)
            ? double(c1_mC1)/dc1 : 0.0;
    };

    // Round 0 => stats after initial labeling
    computeRoundStats(0);

    // Rounds 1..(numiterations-1)
    for (int r = 1; r < numiterations; r++) {
        // copy labels to prevLabels
        for (int i = 0; i < n; i++) {
            prevLabels[i] = labels[i];
        }
        labelPropagationIteration(adj, prevLabels, labels);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i <= GRID_SIZE; i++) {
        for (int j = 0; j <= GRID_SIZE; j++) {
            ...
            // TODO: check if numiterations is correct
